...
public class ExcelWriter {
  ...
  public synchronized boolean setCellData(String sheetName, String colName, int rowNum,
      String data) {
    try {
      fis = new FileInputStream(path);
      ...
      row = sheet.getRow(0);
      for (int i = 0; i < 20; i++) { // fixing max column search value to 20 as, have
                                     // row.getLastCellNum is giving error when we try to write in a
                                     // newly created sheet
  ...
  public synchronized boolean setCellDataColNo(String sheetName, int colNo, int rowNum,
      String data) {
    try {
      fis = new FileInputStream(path);
      workbook = new XSSFWorkbook(fis);
