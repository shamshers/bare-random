...
import com.exlservice.lds.libraries.DataReader;
import com.exlservice.lds.libraries.DynamicQuery;
import com.exlservice.lds.libraries.PublicVariables;
import com.exlservice.lds.testcases.CreateTestCasesFromTestSuite;
...
public class ReadingTestSuiteXLWithRunmode {
  ...
  public static void main(String[] args) throws EmptyStackException, IOException {
    ...
    logger.info(DynamicQuery.CONFIG_EXCEL_FILE_PATH + PublicVariables.SEP
        + PublicVariables.MASTER_TEST_SUITE);
    logger.info(DynamicQuery.CONFIG_EXCEL_FILE_PATH + PublicVariables.SEP
        + PublicVariables.MASTER_TEST_SUITE);
    try {
      masterTestSuiteXL = new XLReader(DynamicQuery.CONFIG_EXCEL_FILE_PATH + PublicVariables.SEP
          + PublicVariables.MASTER_TEST_SUITE);
    } catch (Exception e) {
      logger.error("Exception in main method : The file '" + PublicVariables.MASTER_TEST_SUITE
          + "' cannot be found in the mentioned path - ' " + DynamicQuery.CONFIG_EXCEL_FILE_PATH
          + "'." + e);
    ...
    if (rowCount > 0) {
      for (; rowNum <= rowCount; rowNum++) {
        currentTestSuite =
            (String) masterTestSuiteXL.getCellData(PublicVariables.MASTER_TEST_SUITE_SHEET_NAME,
                PublicVariables.MASTER_TEST_SUITE_1ST_COLUMN, rowNum);
        if (masterTestSuiteXL
            .getCellData(PublicVariables.MASTER_TEST_SUITE_SHEET_NAME,
                PublicVariables.MASTER_TEST_SUITE_RUNMODE_COLUMN, rowNum)
            .equalsIgnoreCase(PublicVariables.RUNMODE_YESVALUE)) {
          try {
            testSuiteWithRunModeY = new XLReader(DynamicQuery.CONFIG_EXCEL_FILE_PATH
                + PublicVariables.SEP + currentTestSuite + ".xlsx");
          } catch (Exception e) {
            logger.error("The file " + currentTestSuite + ".xlsx"
                + "cannot be found in the mentioned path -" + DynamicQuery.CONFIG_EXCEL_FILE_PATH);
            String errMsg = "An exception occurred while reading Master Suite File.";
            logger.error(errMsg + "StackTrace :<br/>" + FwUtil.getStackTrace(e));
          }
          ...
          logger.trace(
              "The path to the current testsuite xl file is " + DynamicQuery.CONFIG_EXCEL_FILE_PATH
                  + PublicVariables.SEP + currentTestSuite + ".xlsx");
      ...
      logger.trace(
          "There are no testsuites in the MasterTestSuite " + PublicVariables.MASTER_TEST_SUITE);
    }
  }
}
